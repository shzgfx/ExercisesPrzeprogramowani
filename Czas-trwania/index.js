...
function filterLessons(lesson, identifier ) {
  const lessonsPart = lesson;
  return lessonsPart.filter(e => (e.name.includes(identifier))).map(e => e.length).map(e =>((e.split(':').map(e => parseInt(e))))).reduce((a,b) => [a[0]+b[0], a[1]+b[1]]).reduce((a,b) => [a + ((b -(b % 60))/60), b%60]).reduce( (a,b) => [((a-a%60)/60), a%60, b]);
}

function percentOf(total = []) {
  const totalMinutes = total[0] * 60 + total[1];
  return function(length) {
    const len = length;
    const partMinutes = length[0] * 60 + length[1];
    return ((partMinutes * 100)/totalMinutes).toFixed(2);
  };
}

function totalDuration() {
  const firstLesson = lessons[0].length.split(':').map(e => parseInt(e)).reduce((a, b) => [0, a, b]);
  const firstPart = filterLessons(lessons, '1x');
  const secondPart = filterLessons(lessons, '2x');
  const thirdPart = filterLessons(lessons, '3x');

  const total = [firstPart, secondPart, thirdPart, firstLesson].reduce((a,b) => [a[0] + b[0], a[1] + b[1], a[2] + b[2]]).reverse().reduce(((a,b,i) => {
    if(!i) {
      a[i] = b%60;
      a[i+1] = (b - b%60)/60;
    } else if (i < 2) {
      a[i+1] = ((a[i] + b) - (a[i]+b)%60)/60;
      a[i] = (a[i] + b)%60;
    } else {
      a[i] += b;
    };
  ...
  const getPartPercent = percentOf(total);
  const partsPercents = [getPartPercent(firstPart), getPartPercent(secondPart), getPartPercent(thirdPart)];

  return `Kurs Opanuj JavaScript trwa ${total[0]} godzin i ${total[1]} minut.
   Moduł Adama: ${firstPart[0]} godzin ${firstPart[1]} minut, jest to około ${partsPercents[0]}% kursu
   Moduł Przemka: ${secondPart[0]} godzin ${secondPart[1]} minut, jest to około ${partsPercents[1]}% kursu
   Moduł Marcina: ${thirdPart[0]} godzin ${thirdPart[1]} minut, jest to około ${partsPercents[2]}% kursu`;
}
